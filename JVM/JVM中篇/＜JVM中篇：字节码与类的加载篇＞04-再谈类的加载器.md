...
```java
复制代码1234567891011121314JAVALauncher.ExtClassLoader var1;
try {
...
注意，这里的Parent类并不是Java语言意义上的继承关系，而是一种包含关系注意，这里的Parent类并不是Java语言意义上的继承关系，而是一种包含关系
...
```java
复制代码12345678910111213141516JAVAclass ClassLoader{
...
```java
复制代码123456789JAVASystem.out.println("＊＊＊＊＊＊＊＊＊＊启动类加载器＊＊＊＊＊＊＊＊＊＊");
...
```java
复制代码123456789JAVASystem.out.println("＊＊＊＊＊＊＊＊＊＊＊扩展类加载器＊＊＊＊＊＊＊＊＊＊＊");
...
```java
复制代码123456JAVA// 获得当前类的ClassLoader
...
```java
复制代码1234567891011JAVA// 运行结果：null
...
```java
复制代码12345678910111213141516171819202122232425262728293031323334JAVApublic class ClassLoaderTest1{
...
复制代码
1
JAVApublic final ClassLoader getParent()
```
...
复制代码
1
JAVApublic Class<?> loadClass(String name) throws ClassNotFoundException
```
...
复制代码
1
JAVAprotected final Class<?> defineClass(String name, byte[] b,int off,int len)
```
...
```java
复制代码12345678910JAVAprotected Class<?> findClass(String name) throws ClassNotFoundException {
    // 获取类的字节数组
...
复制代码
1
JAVAprotected final void resolveClass(Class<?> c)
```
...
复制代码
1
JAVAprotected final Class<?> findLoadedClass(String name)
```
...
复制代码
1
JAVAprivate final ClassLoader parent;
```
...
- 根据传入的类的全限定名返回一个Class对象。该方法在将Class文件加载到内存的同时，会执行类的初始化。
  ...
  复制代码
  1
  JAVAClass.forName("com.atguigu.java.Helloworld");
  ```
...
- 该方法将Class文件加载到内存时，并不会执行类的初始化，直到这个类第一次使用时才进行初始化。该方法因为需要得到一个ClassLoader对象，所以可以根据需要指定使用哪个类加载器。
  ...
  复制代码
  1
  JAVAClassloader cl = ......; cl.loadClass("com.atguigu.java.Helloworld");
  ```
...
```java
复制代码1234567891011121314JAVApublic class ClassLoaderTest {
    public static void main(String[] args) {
