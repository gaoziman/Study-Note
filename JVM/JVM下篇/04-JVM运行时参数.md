...
```shell
复制代码123456789101112131415161718192021222324252627282930313233343536373839404142434445464748495051SHELL> java -help
...
```shell
复制代码12345678910111213141516171819202122232425262728293031323334SHELL> java -X
    -Xmixed           混合模式执行 (默认)
    -Xint             仅解释模式执行
...
```shell
复制代码12SHELL-XX:+<option>  启用option属性
...
```shell
复制代码12SHELL-XX:<option>=<number>  设置option数值，可以带单位如k/K/m/M/g/G
...
复制代码
1
SHELLjava -Xms100m -Xmx100m -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -jar demo.jar
```

**Tomcat 运行 war 包**

```shell
复制代码1234SHELL# linux下catalina.sh添加
...
```shell
复制代码1234SHELL-XX:+PrintCommandLineFlags 程序运行时JVM默认设置或用户手动设置的XX选项
...
```shell
复制代码1234567891011121314151617181920212223SHELL# 栈
...
```shell
复制代码1234SHELL-XX:+HeapDumpOnOutMemoryError 内存出现OOM时生成Heap转储文件，两者互斥
...
ParallelGCThreads={CPU_Count3+(5∗CPU＿Count/8)(CPU_Count<=8)(CPU_Count>8)ParallelGCThreads={CPU_Count(CPU_Count<=8)3+(5∗CPU＿Count/8)(CPU_Count>8)



```shell
复制代码1234567891011121314SHELL# Parallel回收器
...
复制代码123456789101112131415161718192021222324SHELL# CMS回收器
...
```shell
复制代码123456SHELL-XX:+PrintGC <==> -verbose:gc  打印简要日志信息
...
```shell
复制代码12345678SHELL-XX:+TraceClassLoading  监控类的加载
...
```shell
复制代码12345678SHELL-XX:+DisableExplicitGC  禁用hotspot执行System.gc()，默认禁用
...
```java
复制代码123456789101112131415161718JAVApublic class MemoryMonitor {
