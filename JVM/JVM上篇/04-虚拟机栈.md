...
```java
复制代码12345678JAVApublic static void main(String[] args) {
    test();
}
...
```java
复制代码123456789101112131415JAVApublic class StackDeepTest{
...
```java
复制代码123456789JAVApublic class CurrentFrameTest{
    public void methodA(){
...
```java
复制代码123456789101112131415JAVApublic class SlotTest {
...
```java
复制代码12345JAVApublic void testAddOperation(){
    byte i = 15;
    int j = 8;
    int k = i + j;
}
...
```shell
复制代码1234567891011SHELLpublic void testAddOperation();
    Code:
    0: bipush 15
    2: istore_1
    3: bipush 8
...
```csharp
复制代码12345LANGUAGE-CSHARPpublic void testAddOperation() {
    byte i = 15;
    int j = 8;
    int k = i + j;
}
...
复制代码
1
JAVApublic void testAddoperation(); 		Code:	0: bipush 15 	2: istore_1 	3: bipush 8	5: istore_2	6: iload_1	7: iload_2	8: iadd	9: istore_3    10: return
```
...
复制代码
1
JAVAclass Father{    public static void print(String str){        System. out. println("father "+str);     }    private void show(String str){        System. out. println("father"+str);    }}class Son extends Father{    public class VirtualMethodTest{        public static void main(String[] args){            Son.print("coder");            //Father fa=new Father();            //fa.show("atguigu.com");        }    }
```
...
1. 执行引擎遇到任意一个方法返回的字节码指令（return），会有返回值传递给上层的方法调用者，简称
...
复制代码
1
SHELLException table:from to target type4	 16	  19   any19	 21	  19   any
```
