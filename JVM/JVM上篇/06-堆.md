...
复制代码123JAVAException in thread "main" java.lang.OutofMemoryError: Java heap space
...
```java
JAVA// 详细的参数内容会在JVM下篇：性能监控与调优篇中进行详细介绍，这里先熟悉下
...
```java
JAVApublic static StringBuffer createStringBuffer(String s1, String s2) {
...
```java
JAVApublic static String createStringBuffer(String s1, String s2) {
    StringBuffer sb = new StringBuffer();
    sb.append(s1);
    sb.append(s2);
    return sb.toString();
...
```java
JAVApublic class EscapeAnalysis {
...
SHELL-server -Xmx100m -Xms100m -XX:+DoEscapeAnalysis -XX:+PrintGC -XX:+EliminateAllocations
```
