...
复制代码
1
HTML<a th:href="@{/paramController/testServletAPI(username='admin',password='123456')}">通过 Servlet API 获取</a><br/>
```

测试结果

![动画 ](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750569-1857561955.gif)
...
复制代码
1
JAVAusername=admin,password=123456
```
...
```java
复制代码12345JAVA@RequestMapping("/testParam")
...
复制代码
1
HTML<a th:href="@{/paramController/testParam(username='admin',password='123456')}">通过控制器方法形参获取</a><br/>
```

测试结果

![动画  (1)](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750488-1906051413.gif)
...
```java
复制代码12345JAVA@RequestMapping("/testParam2")
...
```html
复制代码123456789HTML<!--为了更直观地在地址栏中看到请求参数，这里使用get类型请求方式-->
...
![动画  (2)](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750764-1066738062.gif)
...
复制代码
1
JAVAusername=hah, password=111111, hobby=Spring,SpringMVC,SpringBoot
```
...
>
> ```java
> 复制代码1234567JAVA@RequestMapping("/testServletAPI2")
...
> 复制代码
> 1
> JAVAusername=sdfg, password=sdfg, hobby=Spring, hobby2=[Spring, SpringMVC, SpringBoot]
> ```
>
...
```java
复制代码12345JAVA@RequestMapping("/testParam3")
...
复制代码
1
JAVAusername=aaaaaaaaa, password=aaaaaaaa, hobby=[Spring, SpringMVC, SpringBoot]
```
...
复制代码
1
JAVAusername=null, password=aaaaaaaa, hobby=[Spring, SpringMVC, SpringBoot]
```
...
复制代码
1
LANGUAGE-INIusername=ss, password=aaaaa, hobby=[Spring, SpringMVC, SpringBoot]
```

关于`@RequestParam`怎么使用，可以看下源码

```java
复制代码1234567891011121314JAVA@Target(ElementType.PARAMETER)
...
![image-20220318220615418](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750726-693089065.png)
...
复制代码
1
HTMLHTTP Status 400 - Required request parameter 'user_name' for method parameter type String is not present
```

经测试，不论是为 username 传空值还是不传值，都是`400`错误

```bash
复制代码12LANGUAGE-BASH/testParam3?user_name=&password=11&hobby=Spring&hobby=SpringMVC
...
![image-20220318221131481](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750275-1811986543.png)
...
复制代码
1
JAVAusername=null, password=1111, hobby=[Spring, SpringMVC]
```
...
![image-20220318222554198](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750735-926597821.png)
...
复制代码
1
JAVAusername=, password=111, hobby=[Spring, SpringMVC]
```
...
```java
复制代码1234567JAVA@RequestMapping("/testParam3")
...
```url
复制代码12URL/testParam3?user_name=&password=asdf&hobby=Spring&hobby=SpringMVC
...
复制代码
1
CONSOLEusername=heh, password=asdf, hobby=[Spring, SpringMVC]
```
...
```java
复制代码1234567JAVA@RequestMapping("/testHeader")
...
复制代码
1
URLhttp://localhost:8080/SpringMVC/paramController/testParam4
```
...
复制代码
1
CONSOLEHost=localhost:8080, test=RequestHeader
```
...
```java
复制代码12345JAVA@RequestMapping("/testServletAPI")
public String testServletAPI(HttpServletRequest request) {
...
![image-20220319113953469](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750420-300984983.png)
...
复制代码
1
HEADERSSet-Cookie: JSESSIONID=C3DFF845C38BF655C02DDA0BD2DD5638; Path=/SpringMVC; HttpOnly
```

后面每次发送请求，`JSESSIONID`的`Cookie`将会放在*请求报文*信息

![image-20220319120344633](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750836-11284900.png)
...
复制代码
1
HEADERSCookie: JSESSIONID=C3DFF845C38BF655C02DDA0BD2DD5638
```
...
```java
复制代码1234567JAVA@RequestMapping("/testCookie")
...
![image-20220319121031037](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750596-1516226042.png)
...
复制代码
1
CONSOLEjSessionId=C3DFF845C38BF655C02DDA0BD2DD5638, test=CookieValue
```
...
```html
复制代码123456789HTML<form th:action="@{/paramController/testBean}" method="post">
    用户名：<input type="text" name="username"><br/>
    密码：<input type="password" name="password"><br/>
...
```java
复制代码12345JAVA@RequestMapping("/testBean")
...
![动画  (3)](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750542-1968071665.gif)

后台日志信息

```console
复制代码12CONSOLEUser{username='aa', password='11', gender='å¥³äºº', age='12', email='123@qq.com'}
...
```java
复制代码1234567JAVA@RequestMapping("/testServletAPI3")
...
```html
复制代码1234HTML<form th:action="@{/paramController/testServletAPI3}" method="post">
    用户名：<input type="text" name="username"><br/>
    <input type="submit" value="测试请求参数">
</form>
```
...
复制代码
1
HTML<a th:href="@{/paramController/testServletAPI3(username='张三')}">通过setCharacterEncoding设置编码</a><br/>
```
...
![image-20220319150515999](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750746-1784061633.png)
...
```java
复制代码123JAVA// 对其进行iso-8859-1解码并重新UTF-8编码
...
```xml
复制代码1234567891011121314151617XML<!--处理编码-->
<filter>
...
![img](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750778-987395189.gif)

> “知其然，知其所以然”

这个神奇的`CharacterEncodingFilter`到底干了什么？我们一起来看下源码一探究竟

![image-20220319160141358](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750369-2025093791.png)
...
![image-20220319160550784](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750751-138813582.png)

可以看到，`CharacterEncodingFilter`类中并没有`doFilter()`方法，那去它的基类中找找吧~

![image-20220319160644342](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750692-449337847.png)

直接读源码

```java
复制代码123456789101112131415161718192021222324252627282930313233343536JAVA@Override
...
```java
复制代码123456789101112131415161718JAVA@Override
...
![02-SpringMVC 获取请求参数](https://img2022.cnblogs.com/blog/2364648/202203/2364648-20220319173750888-2047541711.png)